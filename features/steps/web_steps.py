...
def get_prefix(context: Any) -> str:
    """
    Determine whether the current scenario is testing wishlist or item.
    If the scenario name or feature name contains 'item', use 'item_' prefix,
    otherwise default to 'wishlist_'.
    """
    name = context.scenario.name.lower() + context.feature.name.lower()
    return "item_" if "item" in name else DEFAULT_PREFIX
